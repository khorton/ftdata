...
# TO DO: 1. Fixed 26 Mar 06.
#
#		 2. Add recording of fuel flow.
#
#		 3. Find out and fix why recording is not shutting down cleanly
#
#		 4. Rework the data writing portion of grab_eis_data() to respect the
#			data items defined in EIS4000.data_to_record, rather than having
#			a hard-coded list of data items.

# DONE	 1. If there is a delay between when the script starts, and when the
#			EIS is started, the script tries to record every possible record
#			until it has recorded the number of records missed during the 
#			period when the EIS was not powered.  Need to review the workings
#			of the count timer.
#
#			It now looks the problem is not due to the count timer logic, but
#			rather that the read(blocksize) keeps waiting forever, with no
#			timeout.  So the loop that runs the count does not increment
#			during the period when there is no data.
#
#			Fix - 26 Mar 06.
#			Changed the timeout from 1 second to 0.1 second.  This appears to
#			to have fixed the problem, based on a single test.	Need more
#			testing to confirm.
...
class EIS4000:
	...
	order_label_units = [('TACH','RPM','RPM'),
						 ...
						 ('ALT','Altitude','ft'),
						 ('VOLT','Volt','volt'),
						 ('CARB','Carb Temp','deg F'),
						 ('ROCSGN','Rate of Climb','ft/mn'),
						 ('AUX2','AUX2','?'),
						 ('AUX3','Fuel Pressure','psi'),
						 ('AUX4','Alternator Load','amp'),
						 ('COOL','Water Temp','deg F'),
						 ('ETI','Hour Meter','hr'),
						 ('HRS','Hrs','hr'),
						 ('MIN','Min','mn'),
						 ('SEC','Sec','s'),
	...
	while data_time < (start_time + duration * 60):
		...
		if data_count >= count:
			...
			if match1:
				...
				# if $match_start is too short, grab another chunk, which 
				...
				try:
					...
					+ str(oilt) + '\t' + str(oilp) + '\t' + str(volt) + '\t' + str(aux3) + '\t'\
